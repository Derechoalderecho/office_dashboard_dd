...
export async function fetchUserByFirebaseUid(firebaseUid: string): Promise<ApiUsuario | null> {
  try {
    ...
    console.log(`userAuthService: Buscando usuario con Firebase UID: ${firebaseUid}`);
    ...
    console.log(`userAuthService: Usuarios obtenidos: ${usuarios ? usuarios.length : 0}`);
    
    if (usuarios && usuarios.length > 0) {
      console.log('userAuthService: Primer usuario en la lista:', usuarios[0].id_usuario_firebase);
    ...
    if (!usuario) {
      ...
      console.log(`userAuthService: No se encontró ningún usuario con Firebase UID: ${firebaseUid}`);
      return null;
    }
    
    logger.info(`Usuario encontrado: ${usuario.primer_nombre} ${usuario.primer_apellido} (ID: ${usuario.id_usuario})`);
    console.log(`userAuthService: Usuario encontrado: ${usuario.primer_nombre} ${usuario.primer_apellido}, Rol: ${usuario.rol}`);
    return usuario;
  } catch (error) {
...
export async function getUserRoleFromFirebaseUid(firebaseUid: string): Promise<string | null> {
  try {
    ...
    console.log(`userAuthService: Buscando rol para Firebase UID: ${firebaseUid}`);
    
    const user = await fetchUserByFirebaseUid(firebaseUid);
    console.log('userAuthService: Usuario obtenido:', user ? 'Encontrado' : 'No encontrado');
